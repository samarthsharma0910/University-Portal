...
const GROQ_API_KEY = process.env.GROQ_API_KEY; require("dotenv").config();
const fs = require("fs");
...
// SIGNIN route (CSV + Admin login added)
...
app.post("/chat", async (req, res) => {
    ...
    if (msg.includes("error") || msg.includes("issue") || msg.includes("not working")) {
        return res.json({
            reply: "It seems like a technical issue. Please contact the Registrar Office in Aryabhatta Block (Basement)."
        });
    }

    if (msg.includes("stress") || msg.includes("anxiety")) {
        return res.json({
            reply: "It's okay to feel stressed sometimes. Take a short break, breathe, and focus on one small step at a time 💙"
        });
    }
